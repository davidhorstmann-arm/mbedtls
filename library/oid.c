...
static size_t oid_component_num_bytes(unsigned int value)
{
    size_t num_bytes = 1;
    value >>= 7;
    while (value != 0) {
...
static int oid_component_encode_into(unsigned char **p, unsigned char *bound,
                                     unsigned int value)
{
    size_t num_bytes = oid_component_num_bytes(value);
                                    ...
                                    const char *buf, size_t size)
{
    ...
    encoded_len = oid_component_num_bytes(((unsigned int)component1 * 40) +
    ...
    while (str_ptr < str_bound && *str_ptr != '\0') {
        ...
        size_t num_bytes = oid_component_num_bytes(val);
    ...
    /* Skip past the '.' */
    str_ptr++;
    ret = oid_component_encode_into(&out_ptr, out_bound,
    ...
    while (str_ptr < str_bound && *str_ptr != '\0') {
        ...
        ret = oid_component_encode_into(&out_ptr, out_bound, val);
        if (ret != 0) {
            return ret;
        }
    }
